...
class WSClient:
    def __init__(self, url, bus, exclude_prefix=("debug/",)):
        self.url = url
        self.bus = bus
    ...
    def connect(self):
        ...
        # Handshake
        key = ubinascii.b2a_base64(ubinascii.random(16)).strip()
        hdr = (
            "GET {} HTTP/1.1\r\n"
            "Host: {}\r\n"
            "Upgrade: websocket\r\n"
        ...
        self.sock.send(hdr.encode())

        # Leer respuesta handshake
        resp = self.sock.recv(1024)
        if b"101" not in resp:
            raise OSError("Handshake WebSocket fallido")

        print("✅ WS cliente conectado a broker", self.url)

        # Suscribirse al bus local
        self.bus.subscribe("#", self._on_publish)

    def _on_publish(self, topic, msg):
        # No enviar debug
        for pfx in self.exclude_prefix:
            if topic.startswith(pfx):
                return
        try:
            self.send_binary(topic, msg)
        except Exception as e:
            print("Error enviando a broker:", e)

    def send_binary(self, topic, payload):
        # Empaquetar igual que servidor local
        topic_b = topic.encode()
        frame = bytes([0x00, len(topic_b)]) + topic_b + payload
        self._send_frame(frame)

    def _send_frame(self, payload):
        L = len(payload)
        header = bytearray([0x82])  # FIN + binario
        if L <= 125:
            header.append(L)
        elif L <= 0xFFFF:
            header.append(126)
    ...
    def loop(self):
        """Lazo opcional para recibir mensajes del broker."""
        while True:
            try:
                ...
                print("WS broker desconectado:", e)
                self.sock.close()
                break
...
def _hacer_otras_tareas(): #added
    pass                   #added

# Socket principal (sirve HTTP y puede hacer upgrade a WS en /ws)
import uselect

def _hacer_otras_tareas():
    # Aquí puedes poner tus tareas periódicas, lectura de sensores, etc.
    pass
...
poller = uselect.poll()
poller.register(s, uselect.POLLIN)  # notifica cuando hay cliente esperando
...
try:
    while True:
        # Espera hasta 50 ms por actividad (sin bloquear)
        ...
        if events:
            for sock, event in events:
                if sock is s:
                    try:
                        ...
                        cl.setblocking(True)  # <-- Importante: este sí bloqueante
                        ...
                        if headers.get("upgrade", "").lower() == "websocket" and path == "/ws":
                            ...
                            try:
                                ...
                                print("Cliente WS desconectado:", e)
                            finally:
                                cl.close()
            ...
            # No hay conexiones pendientes → hacer otras tareas
            _hacer_otras_tareas()
